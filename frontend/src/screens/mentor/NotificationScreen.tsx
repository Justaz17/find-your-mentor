...
import { getMentorDashboard, approveBooking, denyBooking, DashboardBooking } from '../../services/mentorDashboardService';
...
const NotificationsScreen = () => {
  ...
  const [pending, setPending] = useState<DashboardBooking[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      setUpcoming(data.upcoming_bookings);
    } catch {}
  ...
  const allEmpty = pending.length === 0 && upcoming.length === 0;
          ...
          <View>
            ...
            {upcoming.length > 0 && (
              <View style={styles.section}>
                ...
                <Text style={styles.sectionSub}>Your confirmed schedule</Text>
                {upcoming.map(b => (
                  <View key={b.id} style={styles.card}>
                    <View style={styles.cardTop}>
                      <View style={styles.dateBox}>
                        <Text style={styles.dateBoxDay}>
                          {new Date(b.slot_start).toLocaleDateString('en-IE', { weekday: 'short' })}
                        </Text>
                        <Text style={styles.dateBoxNum}>{new Date(b.slot_start).getDate()}</Text>
                      </View>
                      <View style={{ flex: 1 }}>
                        <Text style={styles.cardName}>{b.learner_name}</Text>
                        <Text style={styles.cardService}>{b.service_title}</Text>
                        <Text style={styles.cardTime}>{formatTime(b.slot_start)} – {formatTime(b.slot_end)}</Text>
                      </View>
                      <Text style={styles.amount}>{b.amount_paid === 0 ? 'Free' : `€${b.amount_paid}`}</Text>
                    </View>
                  </View>
            ...
            {allEmpty && (
              <View style={styles.emptyWrap}>
                ...
                <Text style={styles.emptySub}>No pending requests or upcoming sessions</Text>
              </View>
            )}
          </View>
        }
