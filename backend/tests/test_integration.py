...
def register_and_login(session, name, email, password, role="learner"):
    ...
    tok = session.post(f"{BASE_URL}/auth/login", json={
...
def setup_mentor(session, name, email, password):
    ...
    # Profile
    prof = session.post(f"{BASE_URL}/mentors/me/profile", headers=mentor["headers"], json={
    ...
    # Service
    svc = session.post(f"{BASE_URL}/services/me", headers=mentor["headers"], json={
...
class TestConcurrentOverlapProtection:
    ...
    def test_second_learner_overlapping_booking_rejected(self, session):
        ...
        learner_a = register_and_login(
            session, "Caoimhe Ni Laoire",
            unique_email("learnerA"), "PassA123!",
        )
        resp_a = session.post(
            f"{BASE_URL}/bookings",
        ...
        learner_b = register_and_login(
            session, "Tadhg O Murchadha",
            unique_email("learnerB"), "PassB123!",
        )
        resp_b = session.post(
            f"{BASE_URL}/bookings",
        ...
        assert "time" in resp_b.json().get("detail", "").lower() or \
               "booked" in resp_b.json().get("detail", "").lower(), (
...
class TestMentorDenialRestoresSlot:
    ...
    def test_denied_booking_restores_slot_to_available(self, session):
        ...
        learner = register_and_login(
            session, "Eoghan Mac Diarmada",
            unique_email("learner3"), "LearnerFlow3Pass123!",
        )
        book_resp = session.post(
            f"{BASE_URL}/bookings",
            headers=learner["headers"],
